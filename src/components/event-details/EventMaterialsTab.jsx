...
export default function EventMaterialsTab({ eventId, eventTypeId }) {
  ...
  const isStockLow = (materialId, quantity) => {
    ...
    // O estoque disponível é o estoque total menos as reservas já feitas
  ...
  const getAvailableStock = (materialId) => {
    const stock = materialStock[materialId];
    if (!stock) return 0;
    return stock.currentStock - stock.reservedStock;
  };
                        ...
                        <div className="flex items-center">
                          ...
                          {isStockLow(material.material_id, material.quantity) && (
                            <span className="ml-2 text-xs text-red-600 font-medium">
                              Estoque disponível: {getAvailableStock(material.material_id)} unidades
                            </span>
                          )}
                        </div>
                      </TableCell>
